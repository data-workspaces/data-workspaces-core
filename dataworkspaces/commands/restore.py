...
def restore_command(workspace:Workspace, tag_or_hash:str,
    ...
    snapshot_names = set([rn for rn in md.restore_hashes.keys() if md.restore_hashes[rn] is not None])
    ...
    result_resources = {rname for rname in restore_set if workspace.get_resource_role(rname)==ResourceRoles.RESULTS}
    result_resources_in_restore_set = result_resources.intersection(restore_set)
    if len(result_resources_in_restore_set)>0:
        if strict:
            raise ConfigurationError("Restore set contains result resources, which cannot be restored. The following are result resources: %s"%
                                     ', '.join(result_resources_in_restore_set))
        else:
            click.echo("Skipping the restore of the following result resources, which are left in their latest state: %s"%
                       ', '.join(result_resources_in_restore_set))
    ...
    if len(removed_names)>0:
        if strict:
            raise ConfigurationError("Resources have been removed from workspace or have no restore hash and strict mode is enabled."+
                                     " Removed resources: %s" % ', '.join(sorted(removed_names)))
        else:
            click.echo("Skipping restore of resources that have been removed from workspace or have no restore hash: %s"%
                       ', '.join(sorted(removed_names)), err=True)
            restore_set = restore_set.difference(removed_names)
    added_names = restore_set.difference(snapshot_names)
    if len(added_names)>0:
        if strict:
            raise ConfigurationError("Resources have been added to workspace since restore, and strict mode enabled."+
                                     " Added resources: %s" %
                                     ', '.join(sorted(added_names)))
        else:
            click.echo("Resources have been added to workspace since restore, will leave them as-is: %s"%
                       ', '.join(sorted(added_names)), err=True)
    ...
    for r in restore_resource_list:
        if not isinstance(r, SnapshotResourceMixin):
            raise InternalError("Resource %s was in snapshot, but is not a SnapshotResourceMixin"%
                                r.name)
