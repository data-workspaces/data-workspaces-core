...
from dataworkspaces.utils.git_utils import get_git_config_param,\
                                           get_remote_origin_url
from dataworkspaces.utils.subprocess_utils import call_subprocess


def deploy_build_command(workspace:Workspace,
    ...
    if isinstance(workspace, git_backend.Workspace):
        ...
        user_email = git_user_email if git_user_email \
                     else get_git_config_param(workspace_dir, 'user.email',
                                               verbose=workspace.verbose)
        user_name = git_user_name if git_user_name \
                    else get_git_config_param(workspace_dir, 'user.name',
                                              verbose=workspace.verbose)
        argv.append('--appendix=RUN git config --global user.email %s; git config --global user.name "%s"' %
                    (user_email, user_name))
        argv.append('dws+'+get_remote_origin_url(workspace_dir,
                                                 verbose=workspace.verbose))
    else:
...
def deploy_run_command(workspace:Workspace,
                       image_name:Optional[str],
